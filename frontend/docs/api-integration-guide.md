...
## 🤝 7가지 결정 포인트 (우리 추천 vs 양보 가능 여부)

| # | 결정 | 프론트 추천 | 양보 | 비고 |
|---|---|---|---|---|
| 1 | 위험도 enum | `"high" \| "medium" \| "low"` | ⚠️ 케이스만 가능 | 대문자 `HIGH`는 NG |
| 2 | 점수 스케일 | `0~100` | ❌ 절대 불가 | 임계값 명확성 |
| 3 | 필드 네이밍 | `camelCase` | ⚠️ snake_case도 OK (어댑터로 매핑) | |
| 4 | 날짜 | ISO 8601 | ❌ 절대 불가 | Unix timestamp NG |
| 5 | 에러 포맷 | `{ code, message, details? }` | ❌ 절대 불가 | 구조화 필수 |
| 6 | 분석 응답 | 동기 (3~5초) | ⚠️ v1은 동기, 10초+되면 Job ID 추가 | |
| 7 | 페이지네이션 | `page` + `pageSize` | ❌ v1은 OK, v2는 cursor | |
