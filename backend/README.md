...
worker는 API 프로세스와 **분리된 `url-validator` 컨테이너에서 단독 실행**됩니다.
(이전에는 FastAPI lifespan에서도 실행됐으나, 이중 실행 방지를 위해 컨테이너 단일화)
