...
- Encoder model ID: `kdt-2-team4-newbiz/kcelectra-smishing-classifier`
...
3. endpoint mode에서는 이전 endpoint 또는 이전 model revision으로 전환한다.
4. serverless mode에서는 이전 model revision 또는 이전 model ID로 전환한다.
5. smoke test를 실행한다.
...
- inference latency
- endpoint error rate
- endpoint cold start 또는 rate limit
- user feedback rate
