...
- 실제 endpoint URL은 코드에 하드코딩하지 않고 환경변수로 관리한다.

## Environment Variables

Endpoint와 모델 metadata는 다음 환경변수로 분리한다.
...
- Endpoint URL이 바뀌는 경우 endpoint 환경변수
- 배포 기록 문서 또는 release note

## Rollback

문제 발생 시 이전 model version 또는 이전 endpoint로 rollback한다.

권장 절차:

1. 문제가 발생한 prediction log를 확인한다.
2. 현재 endpoint URL과 model version을 확인한다.
3. 이전 endpoint 또는 이전 model revision으로 전환한다.
4. smoke test를 실행한다.
5. rollback 시점과 원인을 기록한다.
