...
Deploy wrapper의 책임은 Hugging Face inference API를 호출하고, 결과를 backend가 사용하기 쉬운 공통 응답 형식으로 정규화하는 것이다. 현재 모델팀은 Encoder를 Hugging Face Inference Endpoint 또는 Spaces API로 배포하고, Decoder는 `Qwen/Qwen3-1.7B`를 Hugging Face Inference Providers에서 few-shot chat completion으로 호출한다. 따라서 실제 연결은 `HF_SERVING_TYPE=endpoint`와 `DECODER_API_TYPE=chat_completion`을 기본 흐름으로 둔다.

## Responsibilities

- Backend가 호출할 FastAPI API 제공
- Encoder inference API 호출
- Decoder Qwen chat completion API 호출
- Endpoint 응답 정규화
...
Deploy wrapper는 static miss 이후 AI inference가 필요한 요청을 받아 Encoder Endpoint와 Decoder Provider API를 호출하고, backend가 저장하고 변환하기 쉬운 형태로 응답을 정규화한다.
...
- `HF_SERVING_TYPE=endpoint`: `ENCODER_ENDPOINT_URL`, `HF_TOKEN`으로 Hugging Face 웹 GUI에서 생성한 Encoder Inference Endpoint를 호출한다. Decoder는 기본적으로 Inference Providers chat completion API를 사용한다.
...
모델링 prototype의 `_get_client()`와 같이 deploy wrapper도
`InferenceClient(provider="featherless-ai")` 경로를 사용한다.
...
2. Decoder로 사용할 `Qwen/Qwen3-1.7B`가 Hugging Face Inference Providers에서 호출 가능한지 확인한다.
3. Hugging Face 웹 GUI에서 Encoder 모델의 `Deploy`를 눌러 Inference Endpoint를 생성한다.
4. 생성된 Encoder endpoint URL을 복사한다.
