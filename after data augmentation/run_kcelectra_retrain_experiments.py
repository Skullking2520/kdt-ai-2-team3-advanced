...
DEFAULT_HARD_NEGATIVE_PATH = (
    SCRIPT_DIR / "data" / "generated" / "hard_negative_keywords_normal.csv"
)
DEFAULT_HARD_POSITIVE_PATH = (
    SCRIPT_DIR / "data" / "generated" / "hard_positive_finance_phishing.csv"
)
...
def load_hard_negative_data(path: Path) -> pd.DataFrame:
    match path.suffix.lower():
        case ".csv":
            hard_negative_df = load_csv(path)
        case ".jsonl":
            hard_negative_df = load_jsonl(path)
        case _:
            raise ValueError(f"Unsupported hard-negative format: {path.suffix}")

    non_normal_count = int((hard_negative_df[LABEL_COL] != 0).sum())
    if non_normal_count:
        raise ValueError(
            "Hard-negative data must contain label=0 only. "
            f"Found {non_normal_count} non-normal rows."
        )

    return hard_negative_df


def load_hard_positive_data(path: Path) -> pd.DataFrame:
    match path.suffix.lower():
        case ".csv":
            hard_positive_df = load_csv(path)
        case ".jsonl":
            hard_positive_df = load_jsonl(path)
        case _:
            raise ValueError(f"Unsupported hard-positive format: {path.suffix}")

    non_positive_count = int((hard_positive_df[LABEL_COL] != 1).sum())
    if non_positive_count:
        raise ValueError(
            "Hard-positive data must contain label=1 only. "
            f"Found {non_positive_count} non-positive rows."
        )

    return hard_positive_df


def merge_extra_examples_into_train(
    train_df: pd.DataFrame,
    valid_df: pd.DataFrame,
    test_df: pd.DataFrame,
    hard_negative_df: pd.DataFrame | None,
    hard_positive_df: pd.DataFrame | None,
) -> pd.DataFrame:
    train_columns = list(train_df.columns)
    merge_parts = [train_df]
    heldout_texts = set(valid_df[TEXT_COL].astype(str)) | set(
        test_df[TEXT_COL].astype(str)
    )
    removed_heldout_overlaps: dict[str, int] = {
        "hard_negative": 0,
        "hard_positive": 0,
    }

    if hard_negative_df is not None:
        before_filter = len(hard_negative_df)
        hard_negative_df = hard_negative_df[
            ~hard_negative_df[TEXT_COL].astype(str).isin(heldout_texts)
        ]
        removed_heldout_overlaps["hard_negative"] = before_filter - len(
            hard_negative_df
        )
        merge_parts.append(hard_negative_df.reindex(columns=train_columns))
    if hard_positive_df is not None:
        before_filter = len(hard_positive_df)
        hard_positive_df = hard_positive_df[
            ~hard_positive_df[TEXT_COL].astype(str).isin(heldout_texts)
        ]
        removed_heldout_overlaps["hard_positive"] = before_filter - len(
            hard_positive_df
        )
        merge_parts.append(hard_positive_df.reindex(columns=train_columns))

    merged_df = pd.concat(merge_parts, axis=0, ignore_index=True)
    before_dedup = len(merged_df)
    merged_df = merged_df.drop_duplicates(subset=[TEXT_COL], keep="first")
    removed_duplicates = before_dedup - len(merged_df)
    merged_df = merged_df.sample(frac=1.0, random_state=SEED).reset_index(drop=True)

    print("\nExtra train example merge:")
    hard_negative_size = 0 if hard_negative_df is None else len(hard_negative_df)
    hard_positive_size = 0 if hard_positive_df is None else len(hard_positive_df)
    print(f"hard_negative_rows={hard_negative_size:,}")
    print(f"hard_positive_rows={hard_positive_size:,}")
    print(
        "removed_heldout_overlaps="
        f"{sum(removed_heldout_overlaps.values()):,} "
        f"({removed_heldout_overlaps})"
    )
    print(f"removed_duplicate_texts={removed_duplicates:,}")
    print(f"train_after_merge={len(merged_df):,}")
    print("\nTrain label distribution after extra examples:")
    print(merged_df[LABEL_COL].value_counts().sort_index())
    if hard_negative_df is not None and "keyword_group" in hard_negative_df.columns:
        print("\nHard negative keyword_group distribution:")
        print(hard_negative_df["keyword_group"].value_counts())
    if hard_positive_df is not None and "keyword_group" in hard_positive_df.columns:
        print("\nHard positive keyword_group distribution:")
        print(hard_positive_df["keyword_group"].value_counts())
...
def train_final_model(
    ...
    results_dir: Path,
    wandb_config: WandbConfig,
    hard_negative_df: pd.DataFrame | None = None,
    hard_positive_df: pd.DataFrame | None = None,
) -> ExperimentResult:
    start = time.time()
    ...
    base_train_df = (
        merge_extra_examples_into_train(
            train_df,
            valid_df,
            test_df,
            hard_negative_df,
        ...
        if hard_negative_df is not None or hard_positive_df is not None
        else train_df
    )
    ...
    run = start_wandb_run(
        ...
        run_name,
        {
            ...
            "use_hard_negatives": hard_negative_df is not None,
            "hard_negative_size": (
                0 if hard_negative_df is None else len(hard_negative_df)
            ),
            "use_hard_positives": hard_positive_df is not None,
            "hard_positive_size": (
                0 if hard_positive_df is None else len(hard_positive_df)
            ),
            "device": DEVICE,
            "model_name": MODEL_NAME,
...
def parse_retrain_experiment_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=parse_bool_arg,
        default=False,
        help="If true, merge hard-negative normal samples into train split only.",
    )
    parser.add_argument(
        "--hard-negative-path",
        type=Path,
        default=DEFAULT_HARD_NEGATIVE_PATH,
        help="CSV or JSONL with text,label,keyword_group,source columns.",
    )
    parser.add_argument(
        "--use-hard-positives",
        type=parse_bool_arg,
        default=False,
        help="If true, merge hard-positive phishing samples into train split only.",
    )
    parser.add_argument(
        "--hard-positive-path",
        type=Path,
        default=DEFAULT_HARD_POSITIVE_PATH,
        help="CSV or JSONL with text,label,keyword_group,source columns.",
    )
...
def main() -> None:
    args = parse_retrain_experiment_args()
    set_seed(SEED)
    ...
    hard_negative_df = None
    if args.use_hard_negatives:
        hard_negative_df = load_hard_negative_data(args.hard_negative_path)
        print(f"\nHard negative path: {args.hard_negative_path}")

    hard_positive_df = None
    if args.use_hard_positives:
        hard_positive_df = load_hard_positive_data(args.hard_positive_path)
        print(f"\nHard positive path: {args.hard_positive_path}")
